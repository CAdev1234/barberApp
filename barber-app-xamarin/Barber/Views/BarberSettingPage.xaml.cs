...
namespace Barber.Views
{
    ...
    public partial class BarberSettingPage : ContentPage
    {
        private bool IsInitComponent = true;
        public BarberSettingPage()
        {
        }
        ...
        private async void settingItemClicked(object sender, EventArgs e)
        {
            ...
            else if (title == "Profile Link")
            {
                await Navigation.PushAsync(new ProfileLinkPage());
            }
            else if (title == "Mobile Pay")
            {
                await Navigation.PushAsync(new MobilePayPage());
            }
            else if (title == "Booking Conferences")
            {
                await Navigation.PushAsync(new BookSettingPage());
            }
            else if (title == "Cancellation and No-shows")
            {
                await Navigation.PushAsync(new CancellationNoShowPage());
            }
            ...
            else if (title == "Invite Your Clients")
            {
                await Share.RequestAsync(new ShareTextRequest
                {
